...
public class IndexBase : ComponentBase, IDisposable
{
    ...
    [Inject]
    protected IWebDavClient Client { get; set; }
    ...
    protected List<PlayListItem> PlayListItems { get; private set; } = new();
    ...
    protected string CurrentTime { get; set; } = TimeZero;

    protected string TotalTime { get; set; } = TimeZero;
    ...
    protected async Task LazyLoad(TreeNode<ResourceItem> treeNode)
    {
        ...
        Status = await Client.FetchChildResourcesAsync(treeNode.Item, CancellationToken.None, treeNode.Item.Level, treeNode.Item.Level);
    ...
    private async Task RefreshTreeAsync()
    {
        ...
        Status = ResourceLoadStatus.Unknown;
        Status = await Client.FetchChildResourcesAsync(Root.Item, CancellationToken.None, 0);
        if (Status == ResourceLoadStatus.Ok)
        {
